...
router.get("/new", async (req, res) => {
  try {
    const { dataModel } = await lastItems(
      productModel,
      quantityElementsToDisplay
    );
...
router.get("/list/:category", async (req, res) => {
  try {
    ...
    const { productsData, totalPages, currentPage, filterData } =
...
router.get("/:element/:elementName", async (req, res) => {
  try {
